...
init -950 python in mas_ev_data_ver:
    ...
    _verify_map = {
        ...
        13: MASCurriedVerify(_verify_str, True), 
...
label prompt_menu:
    ...
    python:
        ...
        if len(unseen_events)>0 and not persistent._mas_unsee_unseen:
            ...
            talk_menu.append(("{b}Non visti{/b}", "unseen"))
        if mas_hasBookmarks():
            talk_menu.append(("Segnalibri","bookmarks"))
        talk_menu.append(("Hey, [m_name]...", "prompt"))
        if len(repeatable_events)>0:
            talk_menu.append(("Ripeti conversazione", "repeat"))
        if _mas_getAffection() > -50:
            if mas_passedILY(pass_time=datetime.timedelta(0,10)):
                talk_menu.append(("Ti amo anche io!","love_too"))
            else:
                talk_menu.append(("Ti amo!", "love"))
        talk_menu.append(("Mi sento...", "moods"))
        talk_menu.append(("A dopo", "goodbye")) #BHO
        talk_menu.append(("Ho cambiato idea","nevermind")) #BHO
